function plotStrainRateCrossSVD(data)

%% Plot strain cross at triangulation verticies
%% for each path step in each time frame
...
% create the avi file
mov=avifile('./output/strainRateCrossSVD.avi','compression','none','fps',1,'quality',100);

% display algorithm step entry
display('Plotting interpolated strain rate cross time series');
...
for i3=1:1:data.numSt-1
    ...
    im=reshape(data.imMIP(i3,1,data.chPIV,:,:),data.MIPYLen,data.MIPXLen);
    ...
    for i4=1:1:size(data.pathTriRepSpr{i3}.X,1)
        
        % scale strain crosses by a factor.  Multiplies strain rate
        % to give number of pixels of length of each cross axis.
        sF=0.000002;
               
        pAx1Spr=data.pathTriRepSpr{i3}.X(i4,1)+sF*((data.pSCSVDInterp(i4,i3)/2));
        pAy1Spr=data.pathTriRepSpr{i3}.X(i4,2);
        pAx2Spr=data.pathTriRepSpr{i3}.X(i4,1)-sF*((data.pSCSVDInterp(i4,i3)/2));
        pAy2Spr=data.pathTriRepSpr{i3}.X(i4,2);
        
        qAx1Spr=data.pathTriRepSpr{i3}.X(i4,1);
        qAy1Spr=data.pathTriRepSpr{i3}.X(i4,2)+sF*((data.qSCSVDInterp(i4,i3)/2));
        qAx2Spr=data.pathTriRepSpr{i3}.X(i4,1);
        qAy2Spr=data.pathTriRepSpr{i3}.X(i4,2)-sF*((data.qSCSVDInterp(i4,i3)/2));
        ...
        Qm=[cos(data.betaSVDInterp(i4,i3)) -sin(data.betaSVDInterp(i4,i3)); sin(data.betaSVDInterp(i4,i3)) cos(data.betaSVDInterp(i4,i3))];
        pVecRot1=Qm*pVec1;
        qVecRot1=Qm*qVec1;
        pVecRot2=Qm*pVec2;
        qVecRot2=Qm*qVec2;
...
if data.playMoviesON==1
    implay('./output/strainRateCrossSVD.avi');
end